...
{
    public partial class QuickTestWithAllDataType : System.Web.UI.Page
    {
    ...
    public class Test1Basic
    {
        ...
        public Test1Basic(string connstr, string baseFolder, bool preserveDb1Db3, bool rowInsert, bool rowInsertIgnore, bool rowReplaceInto, bool rowUpdate, bool rowInsertUpdate)
        {
            ...
            foreach (RowsDataExportMode r in Enum.GetValues(typeof(RowsDataExportMode)))
            {
                for (int i = 1; i < 4; i++)
                {
                    string f = Path.Combine(baseFolder, $"sqldump-{r}-{i}.sql");
                    try
                    {
                        if (File.Exists(f))
                        {
        ...
        void RunIndividualTest(RowsDataExportMode mode)
        {
            _currentMode = mode;
...
";
        ...
        public string GetAdvanceDumpSample()
        {
            string sql = @"
-- ----------------------------------------
